...
## Plot PCA Reslts
```{r plot PCA results, include=FALSE}
# plotPCA can't take dds straight
rldds <- rlog(dds)

png(paste0("$prefix", "_PCA.png"))
...
```{r get gene symbols via mapIds, message=FALSE, warning=FALSE}
rslt <- tryCatch(
                    expr = {
...
```{r create volcano plots, include=FALSE}
# png file
png(paste0("$prefix", "_Volcano.png"))
...
```
[adonis result](./DESeq2_adonis.txt)

## make heatmap plots
```{r heatmap, include=FALSE}
# png file
select <- order(rowMeans(counts(dds, normalized=T)), decreasing=T)[1:1000]
pheatmap(assay(rldds)[select,], cluster_rows=T, cluster_cols=T, show_rownames=F, annotation_col=condition, filename=paste0("$prefix", "_Heatmap.png"))
...
dev.off()
```
![heatmap](./DESeq2_Heatmap.png)

[svg heatmap](./DESeq2_Heatmap.svg)

## do pathway analysis
### sometimes it fails, so wrap it in tryCatch()
...
[session info](./DESeq2_sessionInfo.txt)
